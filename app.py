...
if uploaded_file is not None:
    ...
    with st.spinner("🔍 Analyzing image..."):
        ...
        try:
            ...
            # Model output: class 0 (MRI) vs class 1 (Non-MRI)
            non_mri_prob = float(prediction[0][0])  # Probability of Non-MRI
            mri_prob = 1.0 - non_mri_prob           # Probability of MRI
    ...
    if mri_prob >= 0.5:
        predicted_class = 0  # MRI
        confidence = mri_prob
    else:
        predicted_class = 1  # Non-MRI
        confidence = non_mri_prob
    ...
    if predicted_class == 0:
        st.write(f"""
        The model predicts this is **an MRI image** with {confidence:.2%} confidence.
        
        This means the model is {confidence:.1%} certain that the uploaded image 
        is a medical MRI scan based on its learned features.
        """)
    else:
        st.write(f"""
        The model predicts this is **NOT an MRI image** with {confidence:.2%} confidence.
    ...
    with st.expander("🔧 Technical Details"):
        ...
        st.write("\n**Class Mapping:**")
        st.write(f"- Class 0: MRI")
        st.write(f"- Class 1: Non-MRI")
